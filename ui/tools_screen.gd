# tools_screen.gd  (drop-in replacement)
extends Control
...
func _ready() -> void:
	...
	P2P.sync_progress.connect(_on_progress)
	P2P.table_done.connect(_on_table_done)
	P2P.sync_complete.connect(_on_sync_complete)

func _on_p2p_state(s: P2PManager.State) -> void:
	host_btn.disabled = s != P2PManager.State.IDLE
	join_btn.disabled  = s != P2PManager.State.IDLE
	match s:
		P2PManager.State.DISCOVERING:
			status.text = "Hosting… waiting for peer"; status.show()
			info("DISCOVERING")
		...
		P2PManager.State.SHUTTING_DOWN:
			status.text = "Shutting down…"; status.show()
			info("SHUTTING_DOWN")
		P2PManager.State.IDLE:
			status.hide(); list.hide()
			info("IDLE")
		P2PManager.State.SYNCING:
			status.text = "Syncing..."; status.show()
			info("SYNCING")

func _on_progress(table: String, current: int, total: int):
	status.text = "Sync %s  %d/%d" % [table, current, total]
	status.show()

func _on_table_done(table: String):
	info("Table %s finished" % table)

func _on_sync_complete():
	info("All tables synced")
...
func _on_pick_device(index: int) -> void:
	var item: Dictionary = P2P.discovered[index]
